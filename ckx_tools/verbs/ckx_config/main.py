...
"""
Implementation of the 'ckx config' verb.
"""
...
def help_string():
    overview = 'Prepares a (parallel) build directory.\n\n'
    instructions = " \
 'ckx config .' prepares a conventional ros build directory in ./ linked to sources in ./src \n \
...
def init_configured_build(default_underlay, build_dir_="./", source_dir_="./src", underlays_="/opt/ros/groovy", install_prefix_="./install", doc_prefix_="./doc", release_=False, toolchain_="", platform_=""):
    ...
    if not os.path.exists(os.path.join(build_source_dir, "CMakeLists.txt")):
